...
(function () {
  ...
  function setCampo(id, valor) {
    ...
    const el = $(id);
    if (!el) return;
    // Selects (ex.: Estado/UF) sempre começam com um valor padrão, então a
    // regra de "só vazios" não se aplica a eles — sempre atualizamos.
    const ehSelect = el.tagName === 'SELECT';
    if (CONFIG.soPreencherVazios && !ehSelect && el.value && el.value.trim() !== '') return;
    el.value = valor;
  ...
  async function buscarCNPJ(idCampo) {
    ...
    try {
      ...
      if (mapa.logradouro)    setCampo(mapa.logradouro,    [d.descricao_tipo_de_logradouro, d.logradouro].filter(Boolean).join(' ').trim());
      if (mapa.numero)        setCampo(mapa.numero,        d.numero);
      if (mapa.bairro)        setCampo(mapa.bairro,        d.bairro);
      if (mapa.cidade)        setCampo(mapa.cidade,        d.municipio);
      if (mapa.uf)            setCampo(mapa.uf,            d.uf);

      // CEP: BrasilAPI devolve só dígitos (ex.: "68929508") → formata 00000-000
      if (mapa.cep && d.cep) {
        const cepd = digitos(d.cep).substring(0, 8);
        setCampo(mapa.cep, cepd.length === 8 ? cepd.replace(/(\d{5})(\d{3})/, '$1-$2') : cepd);
      }
      ...
      if (tel.length >= 10) {
        if (mapa.ddd) {
          // Layout com DDD e telefone em campos separados
          setCampo(mapa.ddd, tel.substring(0, 2));
          if (mapa.telefone) setCampo(mapa.telefone, tel.substring(2));
        } else if (mapa.telefone) {
          // Layout com um campo só (DDD/TELEFONE) → "(DD) NÚMERO"
          const ddd = tel.substring(0, 2);
          const num = tel.substring(2);
          const numFmt = num.length === 9
            ? num.replace(/(\d{5})(\d{4})/, '$1-$2')
            : num.replace(/(\d{4})(\d{4})/, '$1-$2');
          setCampo(mapa.telefone, '(' + ddd + ') ' + numFmt);
        }
      ...
      // Site: a base pública da Receita normalmente NÃO fornece site.
      // Se algum dia vier (em qualquer um destes campos), preenchemos.
      if (mapa.site) {
        const site = d.site || d.website || d.url || d.homepage || '';
        if (site) {
          const siteFmt = /^https?:\/\//i.test(site) ? site : 'https://' + site;
          setCampo(mapa.site, siteFmt);
        }
      }
